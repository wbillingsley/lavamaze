...
object SnobotChallenge {
  ...
  val levels = Seq(
    Level(name = "Game of Life", stages = Seq(
      ...
      VNodeStage.twoColumn("Still Life")(() => markdown.div(
         s"""
           |Some patterns turn out to be static.
           |
           |Hit play on this, and you shouldn't see much happen. 
           |
           |Until you toggle a cell, in which case chaos will ensue!
           |
           |""".stripMargin),
        () => VNodeStage.card(
      ...
      VNodeStage.twoColumn("Will it stabilise?")(() => markdown.div(
         s"""
           ...
           |
           |(Please don't wait for it ... give it a minute and move on!)
           |
           |""".stripMargin),
        () => VNodeStage.card(
     ...
     VNodeStage.twoColumn("Will it stabilise?")(() => markdown.div(
         s"""
           |On an infinite board, this one would never stabilise, but just keep on growing
           |
           |(But we run out of space here, so it does settle after a few minutes)
           |
           |Again, don't wait for it.
           |
           |Ok, now we've seen how even little programs can get chaotic and out of control, let's try to 
           |learn a bit of coding to try to get something to do what we do want.
           |
           |""".stripMargin),
        () => VNodeStage.card(
          <.div(LifeGame("demo", 50, 50)(
