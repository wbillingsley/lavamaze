...
case class Vec2(x:Double, y:Double) {
  ...
  def boundWithin(p1:Vec2, p2:Vec2):Vec2 = {
    Vec2(
...
object Vec2 {
  ...
  def randomCartesian(w:Double, h:Double):Vec2 = {
    Vec2(Random.nextDouble() * w, Random.nextDouble() * h)
  }

  def randomPolar(centre:Vec2, r:Double):Vec2 = {
    centre + Vec2.randomDir(Random.nextDouble() * r)
  }
